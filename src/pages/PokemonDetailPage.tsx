...
// IMPORTAMOS AS FUNÇÕES DE FAVORITOS AQUI:
import {
  isFavorite,
  addFavorite,
  removeFavorite,
...
export default function PokemonDetailPage() {
  ...
  // 1. NOVO ESTADO PARA CONTROLAR A ESTRELA
  ...
  useEffect(() => {
    async function fetchDetails() {
      try {
        ...
        // 2. ASSIM QUE CARREGAR O POKÉMON, VERIFICA SE JÁ É FAVORITO
  ...
  // 3. FUNÇÃO PARA LIDAR COM O CLIQUE NA ESTRELA GRANDE
  const handleFavorite = () => {
    ...
    */
    ...
    if (favorited) {
      ...
      // Formata os dados da PokeAPI para o padrão que salvamos no storage
      const pokemonToSave = {
        id: pokemon.id,
        name: pokemon.name,
      ...
      <div className="container-main pt-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch max-w-6xl mx-auto">
          ...
          <div className="flex flex-col gap-6">
            <div
              className={`relative flex-1 min-h-112.5 rounded-[56px] overflow-hidden shadow-2xl bg-linear-to-br ${config.gradient} flex items-center justify-center group`}
            >
              {/* ID NO TOPO-ESQUERDO */}
              ...
              {/* 4. ESTRELA ATUALIZADA (CANTO INFERIOR-DIREITO) */}
              <button
                onClick={handleFavorite}
                className={`absolute bottom-8 right-10 z-20 backdrop-blur-md p-4 rounded-3xl shadow-xl hover:scale-110 active:scale-95 transition-all group/star ${
                  favorited
                    ? 'bg-white border-white/50 border' // Fundo branco quando favoritado
                    : 'bg-white/20 border border-white/30 hover:bg-white' // Transparente quando não favoritado
                }`}
              >
                <Star
                  className={`size-7 transition-all ${
                    favorited
                      ? 'text-yellow-500 fill-yellow-500' // Estrela amarela preenchida
                      : 'text-white group-hover/star:text-yellow-500' // Estrela branca vazia
                  }`}
                />
              </button>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                {
                  label: 'Peso',
                  val: `${pokemon.weight / 10}kg`,
                  icon: Weight,
                },
                {
                  label: 'Altura',
                  val: `${pokemon.height / 10}m`,
                  icon: Ruler,
                },
                {
                  label: 'Base XP',
                  val: pokemon.base_experience,
                  icon: Trophy,
                },
              ].map((item, i) => (
                <div
                  key={i}
                  className="bg-white border border-slate-100 p-5 rounded-4xl flex flex-col items-center text-center shadow-sm hover:shadow-md transition-shadow"
                >
            ...
            <div className="mb-10">
              <div className="flex gap-2 mb-4">
                {pokemon.types.map((t: any) => (
                  <span
                    key={t.type.name}
                    className={`${TYPE_CONFIG[t.type.name]?.color} text-white text-[10px] font-black uppercase tracking-[0.15em] px-5 py-2 rounded-xl shadow-lg shadow-black/5`}
                  >
                    {t.type.name}
                  </span>
                ))}
            ...
            <div className="mt-12 pt-8 border-t border-slate-50">
              ...
              <div className="flex flex-wrap gap-2">
                {pokemon.abilities.map((a: any) => (
                  <div
                    key={a.ability.name}
                    className="px-6 py-2.5 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-primary transition-all cursor-default"
                  >
