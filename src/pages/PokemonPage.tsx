...
export default function PokemonPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const detailPromises = data.results.map((p: any) =>
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col selection:bg-primary/10">
      {/* HEADER ULTRA-CLEAN */}
      <header
        className={`sticky top-14.25 z-40 w-full transition-all duration-500 bg-white/80 backdrop-blur-xl border-b border-slate-100 ${
          isScrolled
            ? 'py-4 shadow-[0_2px_20px_-5px_rgba(0,0,0,0.05)]'
            : 'py-12'
        }`}
      >
        <div className="container-main max-w-6xl">
          <div className="flex flex-col gap-8">
            {/* Título com Animação */}
            <div
              className={`text-center transition-all duration-500 transform ${isScrolled ? 'scale-0 h-0 opacity-0 overflow-hidden' : 'scale-100 opacity-100'}`}
            >
      ...
      <main className="grow container-main py-16 flex flex-col items-center">
        {loading ? (
          <div className="flex flex-col items-center py-20 gap-4">
            ...
            <p className="font-bold text-slate-400 tracking-tight text-sm uppercase">
              Sincronizando Pokémons...
            </p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center max-w-sm mx-auto">
            ...
            <p className="text-slate-500 mt-3 leading-relaxed">
              Não encontramos nenhum Pokémon com esses critérios. Tente limpar
              os filtros.
            </p>
            <button
              onClick={() => {
